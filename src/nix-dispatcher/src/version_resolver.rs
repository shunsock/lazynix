...
/// A single result from nix-versions JSON output
...
/// Resolved version info extracted from nix-versions installable field
#[derive(Debug, Clone)]
...
/// Parse the installable field: "nixpkgs/<commit>#<attr>"
...
/// Resolve an exact package version via nix-versions.
///
/// Calls `nix run github:vic/nix-versions -- --json --one '<name>@<version>'`
...
mod tests {
    ...
    fn test_parse_installable_missing_prefix() {
        let result = parse_installable("invalid/5ed6275#go_1_21");
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_installable_missing_hash() {
        let result = parse_installable("nixpkgs/5ed6275");
        assert!(result.is_err());
    }
}
