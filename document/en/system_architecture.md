...
| `lint`     | Validate every declared package (stable + unstable + pinned) with `nix eval`, and verify pinned versions can still be resolved |
...
The `examples/` directory contains real, runnable LazyNix projects such as `examples/python/`. Each example ships two files that readers care about:

- `lazynix.yaml` --- the input the user writes by hand
- `flake.nix` --- the output that `lnix` generates from that input

Both files are committed to the repository. This is a deliberate design choice, not an oversight.

### Why the generated `flake.nix` is committed

LazyNix's core promise, as stated in `README.md`, is that a project can migrate seamlessly to pure Nix by treating the generated `flake.nix` as its source of truth. That promise only becomes credible when readers can open the repository and read the exact Nix code LazyNix produces, side by side with the YAML that produced it. Hiding the generated files behind `.gitignore` would hide the very artifact that backs up the design philosophy. In effect, the examples are executable documentation of what "generated Nix code" actually looks like.

### The trade-off: drift between input and output

Committing generated files introduces a well-known risk: whenever the generator changes but an example is not regenerated, the input (`lazynix.yaml`) and the output (`flake.nix`) can drift out of sync. LazyNix mitigates this with a CI workflow, `.github/workflows/verify_examples_sync.yml`, which regenerates `flake.nix` for every example directory and fails the build if any diff appears. In other words, the invariant "the committed `flake.nix` matches what `lnix` would generate right now" is enforced by CI, not by trust.

### Contributor guidance

When you modify the flake generator (`crates/lnix-domain/src/service/flake/`), regenerate every example before you commit. From the repository root:

```bash
lnix -C examples/python generate
# repeat for every subdirectory under examples/
```

Forgetting this step causes the sync-verification CI job to fail. Do not edit the generated `flake.nix` by hand --- as the `# Generated by LazyNix - DO NOT EDIT MANUALLY` header states, changes belong in `lazynix.yaml`, and the corresponding `flake.nix` is produced by rerunning `lnix`.
